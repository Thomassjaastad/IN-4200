...
int main(int argc, char *argv[])
{
    ...
    if (my_rank == 0)
    {
        ...
        for (int j = 1; j < num_procs; j++)
        {
            if (my_rank == 0)
            {
                MPI_Send(&image_chars[my_m*(j - 1) + remainder_m], my_m*my_n + remainder_m, MPI_UNSIGNED_CHAR, j, 0, MPI_COMM_WORLD);
            }
            else
            {
                MPI_Send(&image_chars[my_m*(j - 1)], my_m*my_n, MPI_UNSIGNED_CHAR, j, 0, MPI_COMM_WORLD);
            }
